...
2. Set your heuristic measure to to predict the most frequent class in your complete example set (i.e. if no split was to be done at the leaf).[^2]
2. start taking examples from your example set and run them down the tree into a leaf node
3. at the leaf node for every attribute update your sufficient statistics
4. label the leaf node with the majority class seen among the examples so far
5. if all examples are not of the same class then we calculate the split evaluation function (like information gap or Gini index) for every attribute in the current attribute set
6. calculating the split evaluation function should give you the attributes with the highest and second highest evaluation functions
7. now calculate the Hoeffding bound (ε), and if the split evaluation function condition is satisfied $\bar{G}_l(X_a) - \bar{G}_l(X_b) > \epsilon$ and $X_a \neq X_{\phi}$
8. If the condition is true then we split the leaf node on $X_a$, if it is not we don't split and continue collecting examples
...
10. at the new leaf we reset the heuristic measure to use the most frequent class at the leaf node and set the sufficient statistics to zero for each attribute
11. Repeat indefinately. 
